...
def started_ride(request):
    if not request.session.get("is_login", None):
        ...
        open_ride = Ride.objects.filter(
            confirmed=False, completed=False, owner=curr_user
        ).order_by("arrive_time")
        confirmed_ride = Ride.objects.filter(
            confirmed=True, completed=False, owner=curr_user
        ).order_by("arrive_time")
        completed_ride = Ride.objects.filter(
            confirmed=True, completed=True, owner=curr_user
        ).order_by("arrive_time")
        context = {
            "open_rides": open_ride,
...
def edit_ride(request, pk):
    ...
    if request.method == "POST":
            ...
            if group.user != request.user:
                ...
                try:
                    old_group = Group.objects.get(
                        user=request.user, groupNum=ride.passengerNum
                    )
                    ride.shared_by_user.remove(group)
        ...
        ride.save()
        mail_content = (
            "<h3> Your Uber order has been canceled!</h3>"
            ...
            "".format(order_id=ride.pk, dest=ride.dest, time=ride.arrive_time)
        )
        send_mail(
...
def search_ride(request):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            for group in groups:
                rides = [
                    ride for ride in rides if group not in ride.shared_by_user.all()
                ]
            for word in addr:
            ...
            rides = [
                ride
                for ride in rides
                if number + ride.get_passenger_num() + 1 <= ride.get_capacity()
            ]
    ...
    context = {"rides": rides, "msg": message, "form": form}
...
def shared_rides(request):
    if not request.session.get("is_login", None):
        ...
        for group in groups:
            open_ride += [
                ride for ride in open_ride_ if group in ride.shared_by_user.all()
            ]
        ...
        for group in groups:
            confirmed_ride += [
                ride for ride in confirmed_ride_ if group in ride.shared_by_user.all()
            ]
        ...
        for group in groups:
            completed_ride += [
                ride for ride in completed_ride_ if group in ride.shared_by_user.all()
            ]
...
def profile_page(request):
    ...
    context = {"user": user, "info": info, "gender": gender}
...
def vehicle_registrate(request):
    ...
    return render(
        request, "dashboard/vehicle_regist.html", context={"form": vehicle_form}
    )
...
def driver_tasks(request):
    ...
    if not hasattr(request.user, "vehicle"):
        ...
        ongoing = Ride.objects.filter(
            confirmed=True, completed=False, vehicle=request.user.vehicle
        ).order_by("arrive_time")

        completed = Ride.objects.filter(
            confirmed=True, completed=True, vehicle=request.user.vehicle
        ).order_by("arrive_time")
        context = {"ongoing": ongoing, "completed": completed, "user": curr_user}
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            rides = [
                ride
                for ride in rides
                if (not ride.vehicleType)
                or ride.vehicleType == request.user.vehicle.vehicleType
            ]

            rides = [
                ride
                for ride in rides
                if ride.get_passenger_num() + 1 <= request.user.vehicle.get_capacity()
            ]

            rides = [
                ride
                for ride in rides
                if (not ride.special_req)
                or ride.special_req == ""
        ...
        if ride.get_passenger_num() + 1 <= request.user.vehicle.get_capacity():
            if (
                ...
                for group in ride.shared_by_user.all():
                    mail_list.append(group.user.email)
                mail_content = (
                    "<h3> Your Uber order has been confirmed!</h3>"
                    "<br>"
                    "<h4> Order information: </h4>"
                    "Order number: {order_id} <br>"
                    "Destination: {dest} <br>"
...
def delete_confirm(request):
    ...
    vehicle = request.user.vehicle
    rides = Ride.objects.filter(confirmed=True, completed=False, vehicle=vehicle)
