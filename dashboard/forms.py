...
class ProfileForm(forms.ModelForm):
    dob = forms.DateField(
        widget=forms.DateInput(
            attrs={"placeholder": "%d/%m/%Y", "type": "date", "class": "form-control"}
        )
    )
...
class RideRequestForm(forms.ModelForm):
    dest = forms.CharField(
        label="Destination",
        widget=forms.Textarea(
            attrs={"cols": "10", "rows": "1", "placeholder": "Enter your address here."}
        ),
        min_length=0,
        max_length=100,
    )
    ...
    passengerNum = forms.IntegerField(
        ...
        widget=forms.NumberInput(
            attrs={"cols": "5", "rows": "1", "placeholder": "Enter number here."}
        ),
        max_value=11,
        min_value=1,
    )
    ...
    def clean_arrive_time(self):
        return self.cleaned_data.get("arrive_time").astimezone()

    def clean_passengerNum(self):
        if self.cleaned_data.get("passengerNum") > 10:
            raise forms.ValidationError("Max 10 people.")
        if self.cleaned_data.get("vehicleType"):
            if (
                self.cleaned_data.get("passengerNum") + 1
                > self.v_info.capacity[self.cleaned_data.get("vehicleType")]
            ):
                raise forms.ValidationError("Too many passenger for this kind of car.")
...
class SearchRide(forms.Form):
    ...
    def clean_start(self):
        ...
        if cleaned_data.get("start"):
            start = cleaned_data.get("start").astimezone()
            if start < datetime.now().astimezone():
                raise forms.ValidationError(
                    "start time or end time should be future times."
                )
    ...
    def clean_end(self):
        ...
        if cleaned_data.get("start") and cleaned_data.get("end"):
            start = cleaned_data.get("start").astimezone()
            end = cleaned_data.get("end").astimezone()
            if end < start:
